    ...
    for (BoostPad boostPad : BoostManager.allBoosts()) {
      ...
      if (thisDistance < shortestDistance) {
        ...
        boolean isTooWideToCar =
            Math.abs(Angles.flatCorrectionAngle(carPosition, input.car.orientation.getNoseVector(), pickupLocation)) > .15;
        boolean isTooWideToTactic =
            Math.abs(Angles.flatCorrectionAngle(
                pickupLocation, Vector3.from(input.car.position, pickupLocation), tacticPosition)) > .15;
