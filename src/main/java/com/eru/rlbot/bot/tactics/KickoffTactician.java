...
public class KickoffTactician extends Tactician {
  ...
  private KickoffTactic selectTactic(DataPacket input, KickoffLocation location) {
    boolean isThrees = Teams.getTeamSize(input.car.team) == 3;
    if (isThrees && (location.station == KickoffStation.RIGHT || location.station == KickoffStation.LEFT)) {
      return shootOnGoal(input, location);
    }

    // Something is broken here now for center left vs left.
    if (!hasPriority(input, location)) {
      if (isThrees && location.station == KickoffStation.CENTER) {
        // consider staying in goal.
      }
      return grabBoost(input, location);
    } else {
      boolean hasTeammates = hasTeammates(input);
      Random random = new Random();
      if (location.station == KickoffStation.RIGHT || location.station == KickoffStation.LEFT) {
        return chooseLeftRightTactic(input, location, hasTeammates, random);
        ...
        return chooseCenterLeftRightTactic(input, location, random);
      }
    }
  }

  private KickoffTactic chooseCenterLeftRightTactic(DataPacket input, KickoffLocation location, Random random) {
    // -1 or 1
  ...
  private KickoffTactic chooseLeftRightTactic(DataPacket input, KickoffLocation location, boolean hasTeammates, Random random) {
  ...
  private KickoffTactic grabBoost(DataPacket input, KickoffLocation location) {
  ...
  private KickoffTactic shootOnGoal(DataPacket input, KickoffLocation location) {
    // Aim for the goal.
