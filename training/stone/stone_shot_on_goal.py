...
from rlbottraining.common_graders.goal_grader import GoalieGrader, StrikerGrader
...
def make_grader():
    return GoalieGrader(timeout_seconds=6)


def make_striker_grader():
...
@dataclass
class Defense(TrainingExercise):
    grader: Grader = field(default_factory=make_grader)
    ball_x: float = 0
    ball_y: float = 10
    ball_z: float = 100
...
class Setup(TrainingExercise):
    grader: Grader = field(default_factory=make_striker_grader)
    ball_x: float = 0
    ball_y: float = 10
    ball_z: float = 100
    ball_vx: float = 0
